...
function displayPlace(array) {
  ...
  container.innerHTML = array.length > 0 ? array
    .map((arr) => {
    ...
    .join("") : `<p class="txt" style="font-size: 4rem">There is not matching record found</p>`;
...
function displayButton (){
  ...
  const buttons = ["All",...new Set(touristPlaces.map((item) => item.country))];
  buttonContainer.innerHTML = buttons.map((button) => {
    return `<button class="btn" data-id="${button}">${button}</button>`
  }).join("")
  ...
  allButtons.forEach((button) => {
    button.addEventListener("click", function() {
      ...
      const filterArray = touristPlaces.filter((item) => item.country === id);
      displayPlace(id === "All" ? touristPlaces : filterArray);
    });
  });
}
...
export { displayPlace , displayButton}
